...
def MO(structure_img_smooth, global_thresh_method, object_minArea, extra_criteria=False, local_adjust=0.98, return_object=False):
    ...
    bw_low_level = remove_small_objects(bw_low_level, min_size=object_minArea, connectivity=1)
...
def MO_low_level_ma(
    structure_img_smooth: np.ma.core.MaskedArray,
    ...
    if global_thresh_method == "tri" or global_thresh_method == "triangle":
        th_low_level = threshold_triangle(structure_img_smooth.compressed())
    elif global_thresh_method == "med" or global_thresh_method == "median":
        th_low_level = np.percentile(structure_img_smooth.compressed(), 50)
    elif global_thresh_method == "ave" or global_thresh_method == "ave_tri_med":
        global_tri = threshold_triangle(structure_img_smooth.compressed())
        global_median = np.percentile(structure_img_smooth.compressed(), 50)
        th_low_level = (global_tri + global_median) / 2

    bw_low_level = structure_img_smooth.data > th_low_level
...
def MO_high_level_ma(
    ...
    lab_low, num_obj = label(bw_low_level, return_num=True, connectivity=1)
    if extra_criteria:
        local_cutoff = 0.333 * threshold_otsu(structure_img_smooth.compressed())
        for idx in range(num_obj):
            single_obj = lab_low == (idx + 1)
            local_otsu = threshold_otsu(structure_img_smooth[single_obj > 0].compressed())
            if local_otsu > local_cutoff:
                bw_high_level[np.logical_and(structure_img_smooth.data > local_otsu * local_adjust, single_obj)] = 1
    else:
        for idx in range(num_obj):
            single_obj = lab_low == (idx + 1)
            local_otsu = threshold_otsu(structure_img_smooth[single_obj > 0].compressed())
...
def MO_ma(
    ...
    bw_low_level = MO_low_level_ma(structure_img_smooth, global_thresh_method, object_minArea, dilate)

    bw_high_level = MO_high_level_ma(structure_img_smooth, bw_low_level, extra_criteria, local_adjust)
