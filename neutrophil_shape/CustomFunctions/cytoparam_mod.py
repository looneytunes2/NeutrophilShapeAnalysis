...
def parameterize_image_coordinates(
    ...
    harmonics coefficients calculated by using ther package aics-shparam_mod.
    ...
    (coeffs_mem, _), (_, _, _, centroid_mem) = shparam_mod.get_shcoeffs(
        image=seg_mem, lmax=lmax, sigma=0, compute_lcc=True, alignment_2d=False
    )

    # Nuclear SHE coefficients
    (coeffs_nuc, _), (_, _, _, centroid_nuc) = shparam_mod.get_shcoeffs(
...
def get_mapping_coordinates(
    ...
    for i, iso_value in enumerate(np.linspace(0.0, 1.0, 1 + np.sum(nisos))):
        ...
        mesh, _ = shtools_mod.get_reconstruction_from_coeffs(coeffs, lrec=2 * lmax)
...
def cellular_mapping(
    ...
    for i, iso_value in enumerate(np.linspace(0.0, 1.0, 1 + np.sum(nisos))):
        ...
        # Get coeffs at given fixed point
        coeffs = coeffs_interpolator(iso_value).reshape(2, lmax + 1, lmax + 1)
        mesh, grid = shtools_mod.get_reconstruction_from_coeffs(coeffs, lrec=2 * lmax)
...
def cellular_mapping_nonuc(
    ...
    # Get interpolator
    coeffs_interpolator = np.linspace(coeffs_ctr_arr, coeffs_mem_arr, nisos)

    representations = []
    for i in range(nisos):
        
        # Get coeffs at given fixed point
        coeffs = coeffs_interpolator[i,:].reshape(2, lmax + 1, lmax + 1)
        mesh, grid = shtools_mod.get_reconstruction_from_coeffs(coeffs, lrec=2 * lmax)
