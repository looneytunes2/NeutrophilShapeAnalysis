...
def twodholefill(thresh, hole_min, hole_max):
    XY = hole_filling(thresh, hole_min, hole_max, fill_2d=True)
    ...
    filled = np.logical_or(XY, np.logical_or(XZ_fill.swapaxes(1, 0), YZ_fill.swapaxes(2,0)))
    return filled
...
def segment_nucleus(img):
    ...
    # step 1: Masked-Object (MO) Thresholding
    thresh_img, object_for_debug = MO(structure_img_smooth, global_thresh_method='tri', object_minArea=1200, local_adjust = 0.98, return_object=True)
...
def segment_caax_final(img):
    ...
    # step 1: Masked-Object (MO) Thresholding
    thresh_img, object_for_debug = MO(structure_img_smooth[:,:,:], global_thresh_method='tri', local_adjust=0.98, object_minArea=1200, return_object=True)
...
def segment_AG_membrane(img):
    #segment the entire body of an AG++ cell signal
    
    ################################
    ## PARAMETERS for this step ##
    intensity_scaling_param = [2, 5]
    gaussian_smoothing_sigma = 1
    ################################
    # intensity normalization
    struct_img = intensity_normalization(img, scaling_param=intensity_scaling_param)
...
def segment_AG_actin(img):
    #use a harsh threshold to detect the actin signal in an AG++ cell
    ...
    #strict MO thresholding for actin signal itself
    thresh_img, object_for_debug = MO(structure_img_smooth, global_thresh_method='tri', extra_criteria = True, local_adjust = 1.60, object_minArea=20,return_object=True)
...
def caax_noise_fill(caax_ch):
    ...
    hist = np.histogram(caax_ch[caax_ch>200], bins=list(range(0,round(np.max(caax_ch)),10)), range=(0,round(np.max(caax_ch))))
    peaks, properties = signal.find_peaks(hist[0],prominence=5000, width=8)
...
def accurate_centroid(temp_im, row, image_name, xyres, zstep, xmincrop, ymincrop, zmincrop,):
    ...
    if len(skimage.measure.regionprops(im_labeled))<1:
        pass
    else:
        # Get properties
...
def segment_cells_rotafter(
    ...
    """
    #file name template
    cell_name = image_name + '_cell_' + str(int(temp_df.cell)) + '_frame_' + str(int(temp_df.frame))
    ...
    #get intensity features
    cell_int = raw_img_ex[0,:,:,:]
    # nucleus_int = im_temp_whole_int[1,:,:,:]
    mem_feat = get_intensity_features(cell_int, seg_rimg[0,:,:,:])
    mem_keylist = [x for x in list(mem_feat) if not x.endswith('lcc')]
    ...
    # get meshes and save
    (cell_coeffs, grid_rec, widestangle, exceptions_list), (image_, cell_mesh, grid, transform) = shparam_mod.get_shcoeffs_mod(        
        image = seg_rimg[0,:,:,:],
        img_name= cell_name,
        lmax = 16,
        xyres = xyres,
    ...
    cell_coeffs['cell'] = cell_name

        
    (nuc_coeffs, grid_rec, nucwa, nuc_list), (image_, nuc_mesh, grid, transform) = shparam_mod.get_shcoeffs_mod(
        image = seg_rimg[1,:,:,:],
        img_name = cell_name,
        lmax = 16,
        xyres = xyres,
    ...
    info = pd.DataFrame({'cell': temp_df.cell,
            ...
            'speed': temp_df.speed,
            'avg_persistence': temp_df.avg_persistence,
            'avg_speed': temp_df.avg_speed
            })
    ...
    writer = vtk.vtkXMLPolyDataWriter()
    writer.SetFileName(savedir + 'Meshes/' + cell_name + '_Cell_Mesh.vtp')
    writer.SetInputData(cell_mesh)
    writer.Write()
    
    writer = vtk.vtkXMLPolyDataWriter()
    writer.SetFileName(savedir + 'Meshes/' + cell_name + '_Nuclear_Mesh.vtp')
    writer.SetInputData(nuc_mesh)
    writer.Write()
...
def segment_cells_rotafter_memonly(
    temp_df,
    raw_img,
    image_name,
    savedir,
    ...
    """
    #file name template
    cell_name = image_name + f'_cell_{int(temp_df.cell)}_frame_{int(temp_df.frame)+1}'
...
def seg_confocal_40x_memonly_fromslices(
    ...
    #segment
    data = segment_cells_rotafter_memonly(
        row,
        raw_img,
        image_name,
        savedir,
...
def segment_cells_rotafter_AG(
    ...
    """
    #file name template
    cell_name = image_name + '_cell_' + str(int(row.cell)) + '_frame_' + str(int(row.frame))
    ...
    # segment cropped image
    seg_rimg = np.zeros([3]+list(raw_img_ex.shape[1:]))
    seg_rimg[0,:,:,:] = segment_AG_membrane(raw_img_ex[0,:,:,:])
    seg_rimg[1,:,:,:] = segment_AG_actin(raw_img_ex[0,:,:,:])
    seg_rimg[2,:,:,:] = segment_nucleus(raw_img_ex[1,:,:,:])
    

    #add the nucleus segmentation to the actin
    seg_rimg[0,:,:,:] = seg_rimg[0,:,:,:] + seg_rimg[2,:,:,:]
    seg_rimg[seg_rimg>0]=255
    
    #ensure there is no actin segmentation outside the membrane segmentation
    seg_rimg[1,:,:,:] = np.logical_and(seg_rimg[0,:,:,:], seg_rimg[1,:,:,:])

    
    # out = np.array(out, dtype = 'bool')
    # fill in the holes after combining cell and nucleus
    hole_max = 1000
    hole_min = 1
    seg_rimg[0,:,:,:] = hole_filling(seg_rimg[0,:,:,:], hole_min, hole_max)
