...
def segment_caax_el4(img):
    ...
    # step 1: Masked-Object (MO) Thresholding
    thresh_img = MO(structure_img_smooth[:,:,:], global_thresh_method='tri', object_minArea=3000, local_adjust = 0.95)
...
def segment_actinhalotag_norot(img):
    ################################
    ## PARAMETERS for this step ##
    intensity_scaling_param = [0]
    ...
    # step 1: Masked-Object (MO) Thresholding
    thresh_img = MO(structure_img_smooth, global_thresh_method='tri', local_adjust=1.4 ,object_minArea=20)
...
def segment_myosinmApple_norot(img):
    ################################
    ## PARAMETERS for this step ##
    intensity_scaling_param = [0]
    gaussian_smoothing_sigma = 1.5
...
def segment_cells_rotafter(
    ...
    seg_rimg[0,:,:,:] = segment_caax_norot(raw_img[0,:,:,:])
    if 'actin' in cell_name:
        seg_rimg[1,:,:,:] = segment_actinhalotag_norot(raw_img[1,:,:,:])
        strr = 'actin'
    if 'Hoechst' in cell_name:
        seg_rimg[1,:,:,:] = segment_nucleus(raw_img[1,:,:,:])
        strr = 'nucleus'
    if 'DNA' in cell_name:
        seg_rimg[1,:,:,:] = segment_nucleus(raw_img[1,:,:,:])
        strr = 'nucleus'
    if 'myosin' in cell_name:
        seg_rimg[1,:,:,:] = segment_myosinmApple_norot(raw_img[1,:,:,:])
        strr = 'myosin'
    ...
    mem_feat = get_intensity_features(raw_img[0,:,:,:], seg_rimg[0,:,:,:])
    mem_keylist = [x for x in list(mem_feat) if not x.endswith('lcc')]
    str_feat = get_intensity_features(raw_img[1,:,:,:], seg_rimg[1,:,:,:])
...
def segment_cells_rotafter_VV_chem(
    ...
    #get image index
    ind = pd.read_csv(ind_dir+'Chemotaxis Image Index.csv')
    ...
    if row.frame<41:
        ...
        cellstack = raw_dir + sorted([x for x in os.listdir(raw_dir) if f'Reflected_t{int(row.frame)+1}' in x and rand in x],
                                     key=lambda x: int("".join([i for i in x if i.isdigit()])))[0]
        structstack = raw_dir + sorted([x for x in os.listdir(raw_dir) if f'Trans_t{int(row.frame)+1}' in x and rand in x],
                                       key=lambda x: int("".join([i for i in x if i.isdigit()])))[0]
    else:
        # direct = raw_dir + chem + '/'
        time = row.frame+1-41
        #read zstacks for the specific time point and channel separately
        cellstack = raw_dir + sorted([x for x in os.listdir(raw_dir) if f'Reflected_t{int(2-(time)%2)}_l{int(math.floor((time+1)/2))}' in x and chem in x],
                                     key=lambda x: int("".join([i for i in x if i.isdigit()])))[0]
        structstack = raw_dir + sorted([x for x in os.listdir(raw_dir) if f'Trans_t{int(2-(time)%2)}_l{int(math.floor((time+1)/2))}' in x and chem in x],
                                     key=lambda x: int("".join([i for i in x if i.isdigit()])))[0]
...
def segment_cells_rotafter_LLS7(
    raw_img,
    row,
    image_name,
    savedir,
    ...
    if 'DNA' in cell_name:
        ...
        strr = 'nucleus'
    if 'GFP++' in cell_name:
        seg_rimg[1,:,:,:] = segment_cytoGFP(raw_img[1,:,:,:])
        strr = 'cytoGFP'
