...
fig = plt.figure(figsize = (chunknum*4,chunknum*2))
fig.patch.set_facecolor('black')
axvids = []
included_chunks = np.zeros((2, chunksq, topchunks.shape[1], topchunks.shape[-2], topchunks.shape[-1]))



# --- Outer 1x2 layout (left grid and right grid) ---
outer = gridspec.GridSpec(
    1, 2,           # 1 row, 2 columns
    width_ratios=[1, 1],
    wspace=0.2      # space between the two groups
)
...
left_gs = gridspec.GridSpecFromSubplotSpec(
    ...
    wspace=0.1,     # spacing inside the 4×4 grid
    hspace=0.1
)
...
for ck, tp in enumerate(['top','bottom']):
    ...
    if ck == 0:
        ...
        textx = 0.67
    fig.text(textx,0.89, ast, color = 'white', fontsize = 20)
    ...
    for i, image in enumerate(curchunks):
        ...
        gridrow = i//4
        gridcolumn = i%4
        ax = fig.add_subplot(axes[gridrow,gridcolumn])
        #actually ad the image
        imsh = ax.imshow(proj_bc[0], cmap = 'gray', animated = True, zorder = 0)
        
        #append images and ax object
        axvids.append(imsh)
        included_chunks[ck,i] = proj_bc
        
        
        if (i == 0) and (ck == 0):
            scalebar_x_displacement = proj_bc.shape[-1]-5
            scalebar_y_displacement = proj_bc.shape[-2]-11
            scalebar_length = 10
            ...
            #scalbar text
            sb_label = ax.text(scalebar_x_displacement-(scalebar_length/resolution)-2,
                                scalebar_y_displacement + 8,
...
ani.save(__file__.split('.')[0] + '.mp4', writer = writer, dpi = 200)#, extra_args=['-vcodec', 'libx264'])
