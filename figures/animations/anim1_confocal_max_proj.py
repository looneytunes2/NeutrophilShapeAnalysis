...
maxproj = np.zeros((fullimshape[0], fullimshape[-2], fullimshape[3]))
...
times = df.FRAME.sort_values().unique()*time_interval
...
eflabels = [['EF OFF','#9e36ba'] if x<181 else ['EF ON','#2ab54f'] for x in range(fullimshape[0]) ]
...
cbar = plt.colorbar(plt.cm.ScalarMappable(cmap=cmap, norm=norm),
                       cax = cbar_ax,
                        ticks = np.linspace(0, frame_range, int((frame_range/6)+1)),
...
cbar.set_ticks(np.arange(0, frame_range+1, mesh_image_interval))
cbar.set_ticklabels((np.arange(0, frame_range+1, mesh_image_interval)*time_interval/60).astype('str'))
...
def animate(i,):
    ...
    if worms is not None:
        worms.remove()
    if i>0:
        currentworms = wormdf[wormdf.frame==i]
