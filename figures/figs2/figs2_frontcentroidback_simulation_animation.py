...
for i, p in enumerate(meshfl):
    ...
    if 'cum_pos' in locals():
        if i == 0:
            ...
            #rear dot
            spherer = Sphere(registrationName=f'Sphere rear')
            spherer.Radius = avgvol/200
            spherer.Center = [0.0, 0.0, 0.0]
            sphererDisplay = GetRepresentation(spherer)
            sphererDisplay.DiffuseColor = [50/255, 127/255, 168/255]
            #center dot
            spherec = Sphere(registrationName=f'Sphere cent')
            spherec.Radius = avgvol/200
            spherec.Center = [0.0, 0.0, 0.0]
            spherecDisplay = GetRepresentation(spherec)
            spherecDisplay.DiffuseColor = [50/255, 127/255, 168/255]
            #front dot
            spheref = Sphere(registrationName=f'Sphere front')
            spheref.Radius = avgvol/200
            spheref.Center = [0.0, 0.0, 0.0]
            spherefDisplay = GetRepresentation(spheref)
            spherefDisplay.DiffuseColor = [50/255, 127/255, 168/255]
        ...
        frontx = cell_coords[cell_coords[:,0] == np.max(cell_coords[:,0])][0][0]
