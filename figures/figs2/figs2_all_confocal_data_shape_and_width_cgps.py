...
aligns = ['Combined_37C_Confocal_PCA_shape','Combined_37C_Confocal_PCA_s5']
...
scale = 0.0008
binlist = [i+'bins' for i in centers.columns.to_list()]
...
for xrow, a in enumerate(binlist):
    for ycol, b in enumerate(binlist):
        ...
        ### add the PC1 labels
        if (bin1=='PC1') & (bin2=='PC1'):
            ax.set_title(bin2, fontsize = 40)#, pad=24)
            ax.set_ylabel(bin1, fontsize = 40)
        ...
        ### remove the diagonal
            ...
            ax.remove()
            print(f'removed {ind1}-{ind2}')
            continue
        ### set the directory for which half of the plot we're on
        elif ind1<ind2:
            basedir = Path('E:/Aaron',aligns[1])
            allorigins = alloriginslist[1]
            flipbins = [bin1, bin2]
            orig = allorigins[int(xrow)][int(ycol-(1+xrow))]
        else:
            basedir = Path('E:/Aaron',aligns[0])
            allorigins = alloriginslist[0]
            ###flip the origins, since they're formatted for the upper
            ###right wedge
            # allorigins = allorigins[::-1]
            flipbins = [bin2, bin1]
            orig = allorigins[int(ycol)][int(xrow -(1+ycol))]

        datadir = basedir.joinpath('Data_and_Figs')
        savedir = basedir.joinpath('Detailed_Balance')
        aerdir = savedir.joinpath('alldatabs')
        centers = pd.read_csv(datadir.joinpath('PC_bin_centers.csv'), index_col=0)
        
        transdf_sep = pd.read_csv(savedir.joinpath(f'{flipbins[0]}-{flipbins[1]}_interpolated_transitions_separated.csv'), index_col=0)
        bsfield_sep = pd.read_csv(aerdir.joinpath(f'{flipbins[0]}-{flipbins[1]}_bootstrapped_{ntrans}_transitions_average_currents.csv'), index_col=0)
        ...
        if a == binlist[0]:
            ...
            if b!=binlist[-1]:
                ...
                ax.set_yticks(np.arange(0.5,nbins+0.5)[[0,(round(nbins/2)-1),-1]],
                             [round(centers[bin1].iloc[x],2) for x in [0,int(round(nbins/2)-1), int(nbins-1)]])
            ...
            ax.set_xticks(np.arange(0.5,nbins+0.5)[[0,(round(nbins/2)-1),-1]],
                         [round(centers[bin2].iloc[x],2) for x in [0,int(round(nbins/2)-1), int(nbins-1)]])

            ax.spines['top'].set_position(('outward', -24))
            
        ### left side tick stuff    
        elif b == binlist[0]:
            ax.set_ylabel(bin1, fontsize = 40)
            if a!=binlist[-1]:
                ax.set_xticks([])
                ...
                ax.tick_params(top=False, labeltop=False, bottom=True, labelbottom=True, labelsize = 16)
                ax.set_xticks(np.arange(0.5,nbins+0.5)[[0,(round(nbins/2)-1),-1]],
                             [round(centers[bin2].iloc[x],2) for x in [0,int(round(nbins/2)-1), int(nbins-1)]])
                print('hi')
                # ax.spines['right'].set_position(('outward', -24))
            ax.tick_params(left=True, labelleft = True, right = False, labelright=False, labelsize = 16)
            ax.set_yticks(np.arange(0.5,nbins+0.5)[[0,(round(nbins/2)-1),-1]],
                         [round(centers[bin1].iloc[x],2) for x in [0,int(round(nbins/2)-1), int(nbins-1)]])


        ### right side tick stuff
        elif b == binlist[-1]:
            ax.set_xticks([])
            ax.set_xticklabels([])
            ax.tick_params(left=False, labelleft = False, right = True, labelright=True, labelsize = 16)
