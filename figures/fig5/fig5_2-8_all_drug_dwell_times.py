...
for i, d in enumerate(differences):
    ...
    print(stats.kruskal(*[q.diffs.to_list() for _, q in quaddf.groupby('quadrant')]))
    dunn = scikit_posthocs.posthoc_dunn(quaddf, val_col = 'diffs', group_col = 'quadrant', p_adjust = 'fdr_bh')
    dunn_melt = pd.melt(dunn.reset_index(), id_vars = ['index'], value_vars = dunn.columns.to_list())
    ## rename melt variables
    dunn_melt = dunn_melt.rename(columns = {'index': 'group1',
                                            'variable': 'group2',
                                            'value': 'p-adj'})
    dunn_melt['Treatment'] = treatments[i+1]
    dunnlist.append(dunn_melt)
    
dunndf = pd.concat(dunnlist, ignore_index = True)
