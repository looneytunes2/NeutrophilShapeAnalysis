...
time_interval = 5
ntrans = 1
mind = 'E:/Aaron/Combined_37C_Confocal_PCA_planar_LLS_Apply/'
...
TotalFrame = FullFrame.merge(allaers[['aer','angular_velocity','cell','time_elapsed','cumulative_time','real_time']],left_on='cell',right_on='cell')
...
for rl in runlengthlist:
    ...
    for i, cell in TotalFrame.groupby('CellID'):
        ...
        cs, runs = utils.get_consecutive_transitions(cell)
        for r in runs:
            c = cs.iloc[r].copy()
...
unique_cells = TotalFrame.CellID.unique()
...
for mm, met in enumerate(metrics):
    for r, rl in enumerate(runlengthlist):
        ...
        ###adjust colormap in case not all cells included
        cmap_adj = [cmap.colors[i] for i, cell in enumerate(unique_cells) if cell in tempsnip.CellID.unique()]
        
        #scatterplot
        sns.scatterplot(data = tempsnip, x = 'aercoef', y = met, hue = 'CellID', palette = cmap_adj, linewidth = 0,
